...
export const knowledgeRelations: KnowledgeRelation[] = [
  ...
  { id:'r-power-elite-continuity', source:'power-2014', target:'concept-elite-continuity', kind:'raises-question', label:L('ставит вопрос о воспроизводстве элит', 'raises the question of elite reproduction', 'ставить питання про відтворення еліт'), note:L('Смена руководителей не доказывает автоматически смену политических практик; непрерывность институтов и элитных механизмов требует отдельной проверки.', 'A change of leaders does not by itself prove a change in political practices; institutional and elite continuity requires separate examination.', 'Зміна керівників сама по собі не доводить зміни політичних практик; безперервність інституцій та елітних механізмів потребує окремої перевірки.'), confidence:'interpretive' },
  ...
  { id:'r-language-rights', source:'language-law', target:'concept-russian-language-rights', kind:'raises-question', label:L('задаёт правовой контекст', 'provides the legal context', 'задає правовий контекст'), note:L('Языковой закон позволяет отдельно проверять доступ к публичным функциям, гарантии частного общения и ошибочное отождествление языка с политической лояльностью.', 'The language law allows separate examination of access to public functions, private-language guarantees, and the mistaken equation of language with political loyalty.', 'Мовний закон дає змогу окремо перевіряти доступ до публічних функцій, гарантії приватного спілкування та хибне ототожнення мови з політичною лояльністю.'), confidence:'documented' },
  ...
  { id:'r-invasion-emigration', source:'full-invasion', target:'concept-population-emigration', kind:'political-context', label:L('усиливает отток населения', 'intensifies population outflow', 'посилює відтік населення'), note:L('Полномасштабная война стала важным контекстом вынужденного перемещения и потери человеческого капитала, но долгосрочные причины эмиграции шире одного события.', 'The full-scale war became a major context for forced displacement and human-capital loss, while long-term emigration has causes beyond a single event.', 'Повномасштабна війна стала важливим контекстом вимушеного переміщення та втрати людського капіталу, але довгострокові причини еміграції ширші за одну подію.'), confidence:'interpretive' },
