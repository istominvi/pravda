...
export const aiTopics: AITopic[] = [
  ...
  },
  {
    ...
    keywords: {
      ru: ['нато', 'расширение', 'бухарест', 'неделимая безопасность', 'военная инфраструктура'],
      en: ['nato', 'enlargement', 'bucharest', 'indivisible security', 'military infrastructure'],
      uk: ['нато', 'розширення', 'бухарест', 'неподільна безпека', 'військова інфраструктура'],
    },
    summary: L('Одновременно держите в поле зрения право выбирать союз и вопрос о том, как шаги воспринимались другой стороной.', 'Keep both the right to choose alliances and the other side’s threat perception in view.', 'Одночасно тримайте в полі зору право обирати союз і питання про сприйняття загрози іншою стороною.'),
    ...
    suggestions: {
      ru: [
        { kind:'clarify', title:'Разделить вопросы', text:'Мы обсуждаем юридическое право Украины выбирать союз или политический вопрос о последствиях этого выбора для общей безопасности?' },
        { kind:'evidence', title:'Показать две нормы', text:'Документы ОБСЕ одновременно подтверждают свободу выбора союзов и принцип неделимой безопасности; Бухарестская декларация задала траекторию будущего членства без даты.' },
        { kind:'boundary', title:'Ограничить вывод', text:'Восприятие угрозы может объяснять мотивацию государства, но само по себе не создаёт юридического права применять силу.' },
      ],
      en: [
        { kind:'clarify', title:'Separate the questions', text:'Are we discussing Ukraine’s legal right to choose an alliance, or the political consequences of that choice for common security?' },
        { kind:'evidence', title:'Show both norms', text:'OSCE texts affirm both freedom to choose alliances and indivisible security; the Bucharest declaration set a future-membership trajectory without a date.' },
        { kind:'boundary', title:'Limit the inference', text:'Perceived threat may explain motivation, but does not by itself create a legal right to use force.' },
      ],
      uk: [
        { kind:'clarify', title:'Розділити питання', text:'Ми обговорюємо юридичне право України обирати союз чи політичні наслідки цього вибору для спільної безпеки?' },
        { kind:'evidence', title:'Показати дві норми', text:'Документи ОБСЄ одночасно підтверджують свободу вибору союзів і принцип неподільної безпеки; Бухарестська декларація задала траєкторію майбутнього членства без дати.' },
        { kind:'boundary', title:'Обмежити висновок', text:'Сприйняття загрози може пояснювати мотивацію, але саме по собі не створює юридичного права застосовувати силу.' },
      ],
    },
  },
  {
