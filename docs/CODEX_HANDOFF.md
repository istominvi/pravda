...
- Каталог статей отсортирован по хронологии. Собственная дата события используется напрямую, а понятие или аргумент получает явно помеченную контекстную дату ближайшего связанного события/документа.
- Номер статьи — вычисляемая позиция в текущем каталоге, а не стабильный идентификатор. Он одинаково показывается в каталоге, Хроно и Связях; вставка более раннего материала автоматически пересчитывает номера, а URL `/articles/:id` не меняется.
